...
int main(int argc, char *argv[])
{
	...
	if(b_load_from_dir) {
		API_FEATURE_LOADER_LOG("Loading rules from directory '%s' for tizen version %s.\n", s_dir_name, s_tizen_version);
		load_from_dir(s_dir_name, s_tizen_version);
		API_FEATURE_LOADER_LOG("Done.\n");
	}
	if(b_load_from_file) {
		API_FEATURE_LOADER_LOG("Loading rules from file(s) matching pattern: '%s' for tizen version %s\n", s_file_name, s_tizen_version);
		load_from_file(s_file_name, s_tizen_version);
		API_FEATURE_LOADER_LOG("Done.\n");
	}
	if(!b_load_additional_rules &&
		!b_load_from_dir &&
		...
		perm_db_configuration_refresh(API_FEATURES_DIR, i_clear_permissions_flag__);
		API_FEATURE_LOADER_LOG("Done.\n");
	}
