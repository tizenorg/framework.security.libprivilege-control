                          ...
                          bool fast)
{
	...
	rules_array = vector_finish(rules_vector);
        ...
        ret = perm_define_permission_5(app_type, s_permission_name, s_tizen_version, (const char **)rules_array, fast);
	if(ret != PC_OPERATION_SUCCESS)
...
bool validate_rules_from_dir(const char const *s_dir)
{
        ...
        for(i = 0; i < num_files; ++i) {
                ...
                ret = validate_all_rules(rules_array);
        ...
        if ((pdir = opendir(s_dir)) != NULL) {
                while ((ent = readdir(pdir)) != NULL) {
                        if (ent->d_type == DT_DIR  ) {
                                asprintf(&realp, "%s/%s", s_dir, ent->d_name);
