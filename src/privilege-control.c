...
API int perm_define_permission(app_type_t app_type,
	...
	ret = rdb_add_permission_rules(base_api_feature_name, s_tizen_ver, s_type_name, smack_rules);
	if (ret != PC_OPERATION_SUCCESS) {
...
API int perm_define_permission_for_tizen_version(app_type_t app_type,
	...
	// Save api feature to the database.
	ret = rdb_add_permission_rules(base_api_feature_name, tizen_version, s_type_name, smack_rules);
	if (ret != PC_OPERATION_SUCCESS) {
		C_LOGE("RDB rdb_add_permission_rules failed with: %d", ret);
		return ret;
	}
		...
		const char * const s_version)
{
	...
	// BELOW CODE IS A WORKAROUND AND SHOULD BE REMOVED AFTER OPTIMIZING API_FEATURE_LOADER
	char* tmp_version = "2.2.1";

	if (NULL != s_version && 0 == strncmp(s_version,"2.3",3)) {
		tmp_version = "2.3";
	}
